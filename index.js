...
const manifest={id:'community.xemxiec.catalog',version:'1.0.0',name:'XemXiec',description:'Xiec • Nuvio movie catalog',resources:['catalog','meta','stream'],types:['movie','series'],idPrefixes:['movie_'],behaviorHints:{configurable:false},catalogs}; const categoryGenres=m=>cats.filter(c=>c.urls.has(normUrl(m.page_url))).map(c=>c.name); const movieGenres=m=>uniq([...asList(m.genres),...categoryGenres(m)]); const isVietsub=m=>m.vietsub===true||movieGenres(m).some(g=>norm(g)===norm('Phim sex Vietsub'));
const sceneCache=new Map(),scenePending=new Map(),trailerCache=new Map(),trailerPending=new Map(),posterCache=new Map(),posterPending=new Map();
...
app.get('/',(req,res)=>res.json({ok:true,service:'Xiec',version:'1.0.0',manifest:'/manifest.json'})); app.get('/manifest.json',(req,res)=>res.json(manifest)); app.get('/healthz',(req,res)=>res.json({ok:true,version:'1.0.0',movies:movies.length,sceneCache:sceneCache.size,trailers:trailerCache.size,posters:posterCache.size})); app.get('/media-status.json',(req,res)=>res.json(mediaStatus())); app.get('/catalog/:type/:id.json',(req,res)=>res.json(['movie','series'].includes(req.params.type)?catalogResponse(req.params.id,'',req.query):{metas:[]})); app.get('/catalog/:type/:id/:path(*)',(req,res)=>res.json(['movie','series'].includes(req.params.type)?catalogResponse(req.params.id,String(req.params.path||'').replace(/\.json$/i,''),req.query):{metas:[]}));
...
app.get('/stream/:type/:id.json',async(req,res)=>{if(!['movie','series'].includes(req.params.type))return res.json({streams:[]});const rawId=String(req.params.id),baseId=rawId.split(':')[0];const m=movies.find(x=>x.id===baseId);if(!m)return res.json({streams:[]});if(rawId.includes(':image:'))return res.json({streams:[]});if(rawId.endsWith(':trailer')){const trailer=m.code?await discoverTrailer(m.code):null;return res.json({streams:trailer?.url?[{name:'Xiec Trailer',title:`🎬 Trailer • ${clean(m.code)}`,url:trailer.url,behaviorHints:{notWebReady:true}}]:[]});}const streams=[],seen=new Set();for(const s of m.streams||[]){if(!s?.url||seen.has(s.url))continue;seen.add(s.url);const h=await publicStreamHealth(s.url);if(!h.ok){console.log('[STREAM-SKIP]',m.id,h.reason);continue;}streams.push({name:'XemXiec',title:`Xiec • ${s.name||`Nguồn ${streams.length+1}`}`,url:s.url,behaviorHints:{notWebReady:true}});}if(m.manifest_url&&!seen.has(m.manifest_url)){seen.add(m.manifest_url);const h=await publicStreamHealth(m.manifest_url);if(h.ok)streams.push({name:'XemXiec',title:`Xiec • ${streams.length?'Nguồn chính':'#1'}`,url:m.manifest_url,behaviorHints:{notWebReady:true}});else console.log('[STREAM-SKIP]',m.id,h.reason);}if(m.mp4_url&&!seen.has(m.mp4_url)){const h=await publicStreamHealth(m.mp4_url);if(h.ok)streams.push({name:'XemXiec',title:'Xiec • MP4',url:m.mp4_url});else console.log('[STREAM-SKIP]',m.id,h.reason);}return res.json({streams});});
async function warmOne(m){if(!m?.code)return;await Promise.allSettled([discoverTrailer(m.code),discoverPoster(m.code),discoverScenes(m.code)]);} async function warmMedia(){const batch=8;for(let i=0;i<movies.length;i+=batch){await Promise.allSettled(movies.slice(i,i+batch).map(warmOne));await new Promise(r=>setTimeout(r,100));}}app.listen(PORT,'0.0.0.0',()=>{console.log(`XemXiec v1.0.0 on ${PORT}`);console.log(`[STATUS] scenes=${sceneCache.size} trailers=${trailerCache.size} posters=${posterCache.size}`);setInterval(()=>console.log(`[STATUS] scenes=${sceneCache.size} trailers=${trailerCache.size} posters=${posterCache.size}`),60000).unref();});