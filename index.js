...
const manifest={id:'community.xemxiec.catalog',version:'1.0.0',name:'XemXiec',description:'XemXiec • clean Nuvio movie catalog',resources:['catalog','meta','stream'],types:['movie','series'],idPrefixes:['movie_'],behaviorHints:{configurable:false},catalogs}; const categoryGenres=m=>cats.filter(c=>c.urls.has(normUrl(m.page_url))).map(c=>c.name); const movieGenres=m=>uniq([...asList(m.genres),...categoryGenres(m)]); const isVietsub=m=>m.vietsub===true||movieGenres(m).some(g=>norm(g)===norm('Phim sex Vietsub'));
...
app.get('/',(req,res)=>res.json({ok:true,service:'Xiec',version:'0.9.35',manifest:'/manifest.json'})); app.get('/manifest.json',(req,res)=>res.json(manifest)); app.get('/healthz',(req,res)=>res.json({ok:true,version:'1.0.0',movies:movies.length,actorImageScraping:false,scenes:sceneCache.size,trailers:trailerCache.size,posters:posterCache.size})); app.get('/catalog/:type/:id.json',(req,res)=>res.json(['movie','series'].includes(req.params.type)?catalogResponse(req.params.id,'',req.query):{metas:[]})); app.get('/catalog/:type/:id/:path(*)',(req,res)=>res.json(['movie','series'].includes(req.params.type)?catalogResponse(req.params.id,String(req.params.path||'').replace(/\.json$/i,''),req.query):{metas:[]}));
